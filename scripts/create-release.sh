...
MAIN_FILE_PATH='accessibility-new-window-warnings.php'
...
RELEASE_BRANCH_NAME=release/v${BUMPED_VERSION}
...
sed -i.bak -E "s/(const ANWW_VERSION = ')[0-9]+\.[0-9]+\.[0-9]+/\1${BUMPED_VERSION}/" "${MAIN_FILE_PATH}"
sed -i.bak -E "s/(\"version\": \")[0-9]+\.[0-9]+\.[0-9]+/\1${BUMPED_VERSION}/" "${PACKAGE_JSON_PATH}"
sed -i.bak -E "s/(Stable tag: )[0-9]+\.[0-9]+\.[0-9]+/\1${BUMPED_VERSION}/" "${README_PATH}"
...
update_wp_requires_version() {
    ...
    if [[ -z "${tested_up_to}" ]]; then
        echo "Error: Could not extract 'Tested up to' version from ${README_PATH}" >&2
        return 1
    fi
    ...
    echo "Setting 'Requires at least' to: ${target_version}"
...
rm -f "${MAIN_FILE_PATH}.bak" "${PACKAGE_JSON_PATH}.bak" "${README_PATH}.bak"
...
echo "https://github.com/equalizedigital/accessibility-new-window-warnings/compare/${MAIN_BRANCH}...${RELEASE_BRANCH_NAME}?title=Release+v${BUMPED_VERSION}"